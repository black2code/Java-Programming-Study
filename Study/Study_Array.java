...
public class Study_Array {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < myArray2.length; i++) { //2행
            for (int j = 0; j < myArray2[i].length; j++) { //3열씩
                System.out.println(myArray2[i][j]);
            }
        }

        for (int[] ints : myArray2 ){ //2차원 배열을 1차원 배열로
            for(int anInt : ints){ //1차원 배열의 원소를 순회
